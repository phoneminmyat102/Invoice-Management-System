...
const deleteInvoiceItem = async (id,i) => {
    ...
    if(id != undefined) {
        let response = await axios.get(`/api/delete_invoice_item/${id}`);
    }  
        ...
        <div class="card__content">
            ...
            <div class="table">
                ...
                <div class="table--items2" v-for="(itemcart,i) in form.invoice_items" :key="itemcart.id">
                    ...
                    <p v-else>0</p>
                    <p @click="deleteInvoiceItem(itemcart.id)" style="color: red; font-size: 24px;cursor: pointer;">
                        &times;
                    </p>
                </div>
                <div style="padding: 10px 30px !important;">
                    <button @click="openModal()" class="btn btn-sm btn__open--modal">
                        Add New Line
                    </button>
                </div>
            </div>
            ...
            <div class="modal__items">
                <ul style="list-style:none;">
                    <li v-for="(item, i) in listProducts" :key="item.id" style="display:grid;grid-template-columns:30px 350px 15px;align-items:center;padding-bottom:5px;">
                        ...
                        <button @click="addCart(item)" style="border:1px solid #e0e0e0;width:35px;height:35px;cursor:pointer;display:inline;">
                            +
                        </button>
                    </li>
                </ul>
